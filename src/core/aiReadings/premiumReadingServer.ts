...
export async function generatePremiumReadingOnServer(body: PremiumReadingRequest | null) {
  ...
  if (process.env.AI_READING_MOCK_RESPONSE === 'true') {
    return {
      generatedAt: new Date().toISOString(),
      model: `${model}-mock`,
      personalityId: personality.id,
      personalityName: personality.name,
      text: buildMockPremiumReadingText(hexagram.name, question),
    };
  }
  ...
  const maxOutputTokens = getOptionalPositiveInteger(process.env.OPENAI_READING_MAX_OUTPUT_TOKENS);
...
function buildMockPremiumReadingText(hexagramName: string, question: string | undefined): string {
  const questionNote = question
    ? `Your test question was received as: "${question}".`
    : 'No test question was provided, so this mock reading is a general outlook.';

  return [
    'Mock premium reading.',
    '',
    `Hexagram focus: ${hexagramName}.`,
    questionNote,
    '',
    'This response was generated by AI_READING_MOCK_RESPONSE=true and did not call OpenAI. Use it to test entitlement checks, app routing, storage, sharing, and display behavior without spending API credits.',
  ].join('\n');
}
